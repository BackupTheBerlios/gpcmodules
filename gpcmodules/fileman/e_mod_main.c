...
static void  _e_mod_fileman_action_cb(E_Object *obj, const char *params);
static void  _e_mod_fileman_cb(void *data, E_Menu *m, E_Menu_Item *mi);
static void  _e_mod_fileman_menu_add(void *data, E_Menu *m);
...
e_modapi_init(E_Module *m)
{
   ...
   if (act)
     {
	act->func.go = _e_mod_fileman_action_cb;
	e_action_predef_name_set(_("Launch"), _("File Manager"), 
				 "fileman", NULL, NULL, 0);
     }
   maug = e_int_menus_menu_augmentation_add("main/1", _e_mod_fileman_menu_add, 
					    NULL, NULL, NULL);
...
/* action callback */
static void
_e_mod_fileman_action_cb(E_Object *obj, const char *params)
{
   E_Zone *zone;
...
/* menu item add hook */
static void
_e_mod_fileman_menu_add(void *data, E_Menu *m)
{
   E_Menu_Item *mi;
